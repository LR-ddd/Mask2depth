...
class DualPathSelfAttentionGate(nn.Module):
    def __init__(self, channels: int, reduction=8):
        super().__init__()
        ...
        self.depth_att = nn.Sequential(
            ...
            nn.ReLU(),
            SpatialAttention(output_channels=channels//reduction),  # 动态设置输出通道
        ...
        self.seg_att = nn.Sequential(
            ...
            SpatialAttention(output_channels=channels),  # 保持通道一致
            nn.Sigmoid()
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 前向传播流程保持不变
        att_depth = self.depth_att(x)
        att_seg = self.seg_att(x)
        task_weights = self.task_fc(x)
        enhanced = x * (att_depth * task_weights[:,0:1] + att_seg * task_weights[:,1:2])
        return enhanced + x
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, output_channels):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(2, output_channels, kernel_size=7, padding=3),  # 移除分组卷积
            nn.Sigmoid()
        )
    
    def forward(self, x):
        # 输入x的通道数可以是任意值（通过平均和最大值压缩）
        avg = torch.mean(x, dim=1, keepdim=True)    # [B,1,H,W]
        max_val, _ = torch.max(x, dim=1, keepdim=True)
        concat = torch.cat([avg, max_val], dim=1)   # [B,2,H,W]
        return self.conv(concat)  # [B,output_channels,H,W]
...
class MultiresConvDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.task_gate = DualPathSelfAttentionGate(dim_decoder)
    def forward(self, encodings: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(num_levels - 2, -1, -1):
            ...
            # 在原有特征融合后添加
            gated_features = self.task_gate(features)
        return gated_features, lowres_features
