...
class SegmentationHead(nn.Module):
    def __init__(self, in_channels, num_classes, threshold=0.5):
        ...
        #     use_gradient_loss=True,  # 是否启用梯度损失
        #     gradient_weight=1.0,  # 边界梯度损失权重
        #     instance_weight=0.5  # 实例分割损失权重
        # )
        ...
        self.loss_fn = InstanceAwareSegmentationLoss(
            use_gradient_loss=True,  # 是否启用梯度损失
            gradient_weight=0.5,  # 边界梯度损失权重
            instance_weight=1.0,  # 实例分割损失权重
            class_weights=class_weights  # 加权交叉熵损失的类别权重
        )
