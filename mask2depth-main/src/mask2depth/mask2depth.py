...
def create_model_and_transforms(
    ...
    # 创建实例分割头
    instance_seg_head = SegmentationHead(in_channels=256, num_classes=12)
...
class DepthPro(nn.Module):
    ...
    # @torch.no_grad()
    # def infer(self, x: torch.Tensor, resize_to_target_size: Optional[Tuple[int, int]] = None, interpolation_mode: str = "bilinear") -> dict:
    #     """
    #     推理阶段，支持深度图和实例分割的输出.
    #
    #     Args:
    #         x (torch.Tensor): 输入图像.
    #         resize_to_target_size (Optional[Tuple[int, int]]): 目标分辨率，用于调整输出.
    #         interpolation_mode (str): 插值方式，默认 "bilinear".
    #
    #     Returns:
    #         dict: 包含推理结果的字典.
    #     """
    ...
    #
    #     # 前向传播
    #     results = self.forward(x, mode="infer")
    #     return results
